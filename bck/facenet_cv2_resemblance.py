    ...
    if len(faces)>0:
        for rect in faces:
            ...
            if face is None: pass
                ...
                feature_vector = resnet(face.unsqueeze(0)).squeeze().detach().numpy().copy()
                name, similarity = label_to_name(feature_vector,dict)
                ...
                #cv2.putText(img_cv2, "{}".format(similarity),(x,y-20), cv2.FONT_HERSHEY_DUPLEX, 1, (255,0,0),2) #類似度記述 (読みづらいので不採用)
                print(name+":{}".format(similarity))#類似度をターミナルに表示
...
def label_to_name(feature_vector,dict):
    ...
    similarity= np.zeros(len(dict))
    for i in range(len(dict)):
        similarity[i] = cosine_similarity(feature_vector, dict[i])

    name_label = np.argmax(similarity)

    #### 正面を向いて顔が大きく映っている写真をface_dictにjpgで保存する
    #### face_dictに用意した番号順に正解を記入
    #### 辞書フォルダ写真から顔を検出できないとエラーになるので注意
    if name_label == 0:
        name = "Tomita Suzuka"
    #elif name_label == 1:
    #    name = "Kanemura Miku"
...
def main(in_dir):
    ...
    #タイトルの表示
    st.title("Who do you resemble?")
    #制作者の表示
    #st.text("Created by your name")
    #アプリの説明の表示
    st.markdown("This app tells the name of the person who resembles you the most.")
